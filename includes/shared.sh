...
CheckIPTableEntry(){
	...
	AddIP(){
		...
		if [ "$_firmware" -eq "0" ] && [ "$cmd" == 'ip6tables' ] ; then
			$cmd -I "$YAMON_IPTABLES" -j "RETURN" -s $ip
			$cmd -I "$YAMON_IPTABLES" -j "RETURN" -d $ip
			$cmd -I "$YAMON_IPTABLES" -j "$groupChain" -s $ip
			$cmd -I "$YAMON_IPTABLES" -j "$groupChain" -d $ip
		else
			$cmd -I "$YAMON_IPTABLES" -g "$groupChain" -s $ip
			$cmd -I "$YAMON_IPTABLES" -g "$groupChain" -d $ip
	...
	ChangeMACGroup(){
		...
		for line in $matchingMACs ; do
			...
			for rule in $matchingRules ; do
				...
				$cmd -R ${YAMON_IPTABLES} $ln -j $groupChain
