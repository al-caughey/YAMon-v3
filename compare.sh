...
do
    ...
    lmd5=$(echo $(md5sum "$path")| cut -d' ' -f1)
    echo -n "$ts"
    echo -n "$pad"
    if [ "$smd5" == "$lmd5" ] ; then
        echo "    matches"
...
if [ "$needsRestart" == "1" ] ; then
    echo "
One or more files were updated. Would you like to restart now?"
    ...
    if [ "$resp" == 'r' ] || [ "$resp" == 'R' ] ; then
        ...
        echo "
You will have to manually restart soon."
    fi
...
All of your files are up-to-date."
    ...
    echo "
One or more of your files is out-of-date and should be updated."
fi

echo "
Please send questions or commments to questions@usage-monitoring.com
